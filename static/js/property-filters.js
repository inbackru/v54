...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (advancedButton && advancedPanel) {
        advancedButton.addEventListener('click', function(e) {
            ...
            console.log('✅ Advanced filters panel toggled:', !advancedPanel.classList.contains('hidden'));
        });
...
window.handleRoomFilterChange = function() {
    ...
    // Применяем фильтры мгновенно через AJAX (для чекбоксов без debounce)
    window.applyFilters();
};
...
window.applyFilters = function() {
    ...
    // Search query (from property-search input)
